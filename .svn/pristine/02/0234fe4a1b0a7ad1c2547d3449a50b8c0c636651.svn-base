...
function LayerGameRule:init(func) 
    ...
    self.panel_bg:setPosition(ccp(AppConfig.DESIGN_SIZE.width / 2, AppConfig.DESIGN_SIZE.height / 2 - 40))
    ...
    --标题
    local titleSp = loadSprite("title_rule.png")
...
function LayerGameRule:addRuleMsg() 
    ...
    self:runAction(CCCallFunc:create(function()
        ...
        local RuleTxt = require("bull/GameRuleText")
...
function LayerGameRule.put(super, zindex, func)
	if(LayerGameRule.instance == nil) then
		local layer = LayerGameRule.new()
		super:addChild(layer, zindex)
		layer.layer_zIndex = zindex
		layer:init(func)
		
		LayerGameRule.instance = layer
	end
    return LayerGameRule.instance
end
