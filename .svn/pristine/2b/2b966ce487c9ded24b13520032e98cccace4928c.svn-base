--TutorialLogic_GDMJ.lua

TutorialLogic_GDMJ = TutorialLogic_GDMJ or {}
...
-- 初始化教程记录(测试用)
function TutorialLogic_GDMJ:resetRecorder(t)
    -- print("TutorialLogic_GDMJ:resetRecorder")
...
-- 读取教程记录
function TutorialLogic_GDMJ:getRecorder(panel)
    ...
    local path = require("gdmj/GameDefs").CommonInfo.Img_Path
...
-- 更新教程记录
function TutorialLogic_GDMJ:setRecorder(t)
    -- print(os.time(), "记录教程 "..TutorialName[t])
    -- print("TutorialLogic_GDMJ:setRecorder", os.time(), t)
    if t == 1 then
...
-- 发牌教程判定 id:1
function TutorialLogic_GDMJ:checkSend(panel)
...
-- 摆牌教程判定 id:2
function TutorialLogic_GDMJ:checkOrder(panel)
...
-- 双击摆牌教程判定 id:3
function TutorialLogic_GDMJ:checkAutoOrder(panel)
...
-- 创建教程
TutorialLogic_GDMJ.createTutorial = {
    -- 出牌教程(已取消)
    [1] = function()
        --[[
    ...
    [2] = function()
        ...
        array2:addObject(CCCallFuncN:create(function(sender)        
            TutorialLogic_GDMJ:hideTutorial(require("gdmj/Game/GameLogic"):getInstance():getMyPanel(), 2)
        end)) 
    ...
    [3] = function()
        ...
        array2:addObject(CCDelayTime:create(TutorialAutoCloseTime))
        array2:addObject(CCCallFuncN:create(function(sender)        
            TutorialLogic_GDMJ:hideTutorial(require("gdmj/Game/GameLogic"):getInstance():getMyPanel(), 3)
        end)) 
        layer:runAction(CCSequence:create(array2))
        return layer
    end
...
-- 显示教程
function TutorialLogic_GDMJ:showTutorial(panel, t)
...
-- 关闭指定教程
function TutorialLogic_GDMJ:hideTutorial(panel, t)
...
-- 关闭所有教程
function TutorialLogic_GDMJ:hideAllTutorials(panel)
...
-- 教程检查
function TutorialLogic_GDMJ:check(panel, t)
    -- print("--------------------------------------")
...
TutorialLogic_GDMJ.checkPoint = {
    [0] = function(panel)
        TutorialLogic_GDMJ:hideAllTutorials(panel)
    end,
    [1] = function(panel)
        TutorialLogic_GDMJ:checkSend(panel)
    end,
    [2] = function(panel)
        TutorialLogic_GDMJ:hideTutorial(panel, 1)
        TutorialLogic_GDMJ:checkOrder(panel)
    end,
    [3] = function(panel)
        TutorialLogic_GDMJ:hideTutorial(panel, 2)
        TutorialLogic_GDMJ:checkAutoOrder(panel)
    end,
    [4] = function(panel)
        TutorialLogic_GDMJ:hideTutorial(panel, 3)
    end,
    [5] = function(panel)
        if panel.isStart then
            local sLogic = require("Lobby/Set/SetLogic")
            if sLogic.getOutCardType() == 1 then TutorialLogic_GDMJ:hideTutorial(panel, 1) end
            if sLogic.getCardOrderType() == 1 then TutorialLogic_GDMJ:hideTutorial(panel, 2) end
            if not TutorialLogic_GDMJ:checkSend(panel) then
                TutorialLogic_GDMJ:checkOrder(panel)
            end
        end
    end
