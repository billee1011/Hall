...
function LayerGame:init(playerNum,gameLogicInstance) 
    ...
    self.downIndex = 9 --一行显示倒下去个数
    if self.player_num == 3 then
        self.send_cout = {4, 4, 4, 4, 4, 4, 4, 4, 4, 2, 1, 1}
        self.downIndex = 9
    end

    require("gdmj/Game/GameLogic").loadingCache()
    ...
    local versionlab = CCLabelTTF:create( require("gdmj/GameVersion").new().version , 
    ...
    --标题
    local titleSp = loadSprite("gdmj/gameTitle.png")
...
function LayerGame:initGameUI() 
	...
	CommonInfo.Logic_Path = "gdmj/Game/GameLogic"
    -- 聊天
    self:initVoiceUI()
...
function LayerGame:initPlayUI()
    CommonInfo.Logic_Path = "gdmj/GamePlayLogic"
...
function LayerGame:initFriendGameUI() 
    if not self.residue_friendgame then
		...
		local norSp = loadSprite("gdmj/GameOption_btn_nor.png", true)
		local clickSp = loadSprite("gdmj/GameOption_btn_down.png", true)
		local disableSp = loadSprite("gdmj/GameOption_btn_nor.png", true)
		...
		self.gameOption = CCButton.createCCButtonByStatusSprite(norSp,clickSp,disableSp,function()
			require("gdmj/Game/LayerGameOption").create(self,self.freindresult_zIndex)			
		...
		self.gameOptionTip = loadSprite("gdmj/GameOption_tip.png")
...
function LayerGame:initFriendGameRule() --添加规则文本
	...
	local ruleData = FriendGameLogic.getRuleValueByIndex(3) -- 游戏类型
	table.insert(marksSp, loadSprite("deskRule/gameType"..ruleData..".png"))
--	table.insert(marksSp, loadSprite("deskRule/playerNumber_"..self.player_num..".png"))--人数
	...
	table.insert(marksSp, loadSprite("deskRule/magic"..magicCardIndex..".png"))--鬼牌
	if notMagicDouble == 0x200 then --无鬼翻倍
		table.insert(marksSp, loadSprite("deskRule/NotMagicDouble.png"))
	end
	
	--马牌数量
	ruleData = FriendGameLogic.getRuleValueByIndex(5)
	table.insert(marksSp, loadSprite("deskRule/horse"..2*(ruleData - 1)..".png"))
	
	--马分
	ruleData = FriendGameLogic.getRuleValueByIndex(6)
	if ruleData == 1 then
		table.insert(marksSp, loadSprite("deskRule/horseAnd_"..ruleData..".png"))
	end

	local xspace = 22
	local y = CommonInfo.View_Height / 2 + 240
    local scale = 0.7
	local function addDianMark(y)
        --添加逗号
        local dianSp = loadSprite("deskRule/,.png")
        dianSp:setAnchorPoint(ccp(0, 1))
        dianSp:setPosition(ccp(CommonInfo.View_Width / 2 - 305 + xspace, y))
		dianSp:setScaleX(scale)
        xspace = xspace + dianSp:getContentSize().width * scale + 6
   ...
   for i,v in ipairs(marksSp) do
        ...
        v:setPosition(ccp(CommonInfo.View_Width / 2 - 305 + xspace, y))
		v:setScaleX(scale)
        xspace = xspace + v:getContentSize().width * scale
        self:addChild(v)              
    end
	--具体玩法
	marksSp = {}
	...
	for i,v in ipairs(optionName) do
		if Bit:_and(gameOption,options[v]) ~= 0 then
			table.insert(marksSp, loadSprite("deskRule/Option_"..v..".png"))
		end
	end
	y = y - 30
	for i,v in ipairs(marksSp) do
    ...
    if not self.invite_btn then
        self.invite_btn = CCButton.put(self, CCButton.createCCButtonByFrameName("mjdesk/btnInviteFriend1.png", 
                ...
                function()
                    ...
                    --微信好友
                    local msg, titile, roominfo = require("gdmj/LayerDeskRule").getInviteMsg()
                    if now > 0 and all > now then 
...
function LayerGame:initCardPanle(banker)
    ...
    local cardnums, carduis = {17, 17, 17, 17}, {}
    if self.card_count ~= 136 then
    ...
    for i,v in ipairs(zindex) do
		...
		self.player_panel[chair + 1]:setTag(v)
        --创建牌墙
        if self.player_num == 4 then
			...
			carduis[v][1]:getParent():setVisible(false)
        end
    end
...
function LayerGame:addPlayerStaticDiscardCard(chair, banker, cbDiscardCard, bGameing)
	...
	local cardSpList = {{},{},{},{}} --打出去牌的精灵，保存这个这提示时有用
	local cardSp = nil
    for i,v in ipairs(cbDiscardCard) do
        for j,w in ipairs(v) do
            cardSp = self.player_panel[i]:addStaticPassed(w)
			table.insert(cardSpList[i],{w,cardSp})
        end
...
function LayerGame:showPlayerLocation() 
    local GameLogic = require("gdmj/Game/GameLogic"):getInstance()
...
function LayerGame:addHorsePanel(birds, valids, func) --新的抓马动画
    require("gdmj/Game/LayerHorseResult").put(self, self.freindresult_zIndex)
...
function LayerGame:waitForPiao(scese)
    if not self.result_panel then
        ...
        elseif require("gdmj/Game/GameLogic").isLogicExist() then
            self.game_start:setVisible(true)
            require("gdmj/Game/GameLogic"):getInstance():onEnterGameView()
        end
    end
...
function LayerGame:setTimerDirct(index)
	if self.player_num == 3 then
		self.timer_dirct = loadSprite("mjdesk/mjTimerDirct3_"..index..".png")
		self.timer_dirct:setScale(0.67)
	else
...
function LayerGame:playMagicCardAnima(magicCard, backfunc)
	...
	local function addCard(cardData,fun)
		...
		array:addObject(CCDelayTime:create(0.05))--停留几秒
		array:addObject(CCCallFunc:create(function()
    ...
    ----然后不断产生随机牌 
    for i=1,10 do
		local index = math.random(self.card_count == 136 and 34 or 27)
        addCard(card[index],nil)
    end
	local tempMagicCard = magicCard
	if magicCard <= 9 then --1-9万
		tempMagicCard = (magicCard - 1) < 0x01 and 0x09 or (magicCard - 1)
...
function LayerGame:addMagicCard(magicCard) --添加万能牌
	...
	local laizi_mark = loadSprite("mjValue/markGui.png")
...
function LayerGame:popMessageBox(msgText,callBack,super,zIndex)--消息提示框
	if self.tip_dlg ~= nil then 
		self.tip_dlg:removeFromParentAndCleanup(true)
        self.tip_dlg = nil
	end
	self.tip_dlg = require("Lobby/Common/LobbySecondDlg").putTip(super, zIndex, msgText, function()
		if self.tip_dlg then
			self.tip_dlg:removeFromParentAndCleanup(true)
			self.tip_dlg = nil
		end
...
function LayerGame:showWaringTip(chairID)--显示警告提示
	if self.gameLogicInstance.user_list == nil or self.gameLogicInstance.user_list[chairID] == nil then
		return
	end
	local name = self.gameLogicInstance.user_list[chairID]._name
   	local msg = string.format("【%s】9张落地报警",name)
	require("HallUtils").showWebTip(msg,nil,nil,ccp(640,430),2)	
end
