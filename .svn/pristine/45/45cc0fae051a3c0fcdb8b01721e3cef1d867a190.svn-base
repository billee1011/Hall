    ...
    if layerChat and layerChat.view then
        ...
        if i ~= layerChat.currentPanel then
            ...
            for j = 1, 3 do
                ...
                local s = (i == opt.id) and "sgmj/labelbg%d_selected.png" or "sgmj/labelbg%d_normal.png"
...
function LayerChat:addEmoteDetail()
    ...
    local title = loadSprite("sgmj/chatLab1.png")
...
function LayerChat:addChatDetail()
    ...
    local titleSz = titleBg:getContentSize()
    local title = loadSprite("sgmj/chatRTitleBg.png")
    title:setPosition(ccp(titleSz.width / 2, titleSz.height / 2))
    titleBg:addChild(title)
        ...
        if index == 1 then
            ...
            tableView:setCreateCellFunc(function(idx)
                ...
                if idx < #quickChatCfg - 1 then
                    local sp = loadSprite("sgmj/split.png")
            ...
            tableView:setCreateCellFunc(function(idx)
                ...
                if chat then
                    ...
                    for k, v in pairs(chat) do
                        local chair = require("sgmj/Game/GameLogic"):getInstance():getRelativeChair(k)
    ...
    for i,v in ipairs(poses) do
        local btn = CCButton.put(titleBg, CCButton.createCCButtonByFrameName("sgmj/chatLab"..(i + 1).."_normal.png", 
        "sgmj/chatLab"..(i + 1).."_select.png", "sgmj/chatLab"..(i + 1).."_select.png", function(tag, target)
        ...
        local btnSp =  loadSprite("sgmj/chatRTitle.png")
...
function LayerChat:addTableView(ancpos, pos)
    ...
    -- 背景框
    local viewbg = loadSprite("sgmj/chatInnerBg.png", true)
    ...
    --添加标题
    local titleBg = loadSprite("sgmj/chatLTitle.png")
...
function LayerChat:init()
    ...
    self.bg = loadSprite("sgmj/chatBg.png", true)
    ...
    self.input = input
    local btn_input = CCControlButton:create(loadSprite("sgmj/btn_send.png", true))
    btn_input:setBackgroundSpriteForState(loadSprite("sgmj/btn_send_press.png", true), CCControlStateHighlighted)
