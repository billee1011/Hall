...
setmetatable(GamePlayLogic, require("gdmj/Game/GameLogic"))
GamePlayLogic.__index = GamePlayLogic 

function GamePlayLogic:new()  
  local self = require("gdmj/Game/GameLogic"):new()
...
function GamePlayLogic:onGamEnd(data)
	...
	self.gameEndInfo.allPayUser = data.AP   
    ...
    for i,v in ipairs(data.UL) do
        ...
        local score = {nTotoalScore = v.GSL[1], nHuScore = v.GSL[2], nGangScore = v.GSL[3], 
                nHorseScore = v.GSL[4], nContinuousWinScore = v.GSL[5],nPiaoScore = v.GSL[6],nCallBankerScore = v.GSL[7]}
        table.insert(self.gameEndInfo.UserScoreList, score)  

        table.insert(self.gameEndInfo.gameScores, score.nTotoalScore - score.nHorseScore)                
    end
end
