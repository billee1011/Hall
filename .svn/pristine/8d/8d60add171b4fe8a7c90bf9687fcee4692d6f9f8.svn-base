...
function SpriteWestCard:createHorizontalDirectArrow(dirct)
	--[[if dirct and dirct >= 1 and dirct <= 3 then --左边玩家的碰牌
		--方向指示箭头 指示是碰谁的牌
		local rotation = {195,95,15}
		...
		self.directArrowSp:setPosition(ccp(self.base_size.width + 10, self.base_size.height/2))
	end]]
end
function SpriteWestCard:initHorizontalPassed(index, ctype,dirct)
	...
	self:createHorizontalDirectArrow(dirct)
...
function SpriteWestCard:initVerticalPassed(index, ctype,dirct)
