...
function GameLibSink:run(logonIP,logonPort,webRoot)
  	...
  	self.game_lib:createGamelib(GameDefs.CommonInfo.Game_ID, GameDefs.CommonInfo.Game_Version,
  	 	self, self.client_sink, require("AppConfig").CONFINFO.PartnerID, require("GameConfig").getGameVer("k510"),
...
function GameLibSink:onLogonFinished(lUserDBID,szName,szPass)
	cclog("onLogonFinished")
    local gamelib = require("k510/GameLibSink").game_lib
    -- gamelib:autoEnterGameRoom("普通场")
...
function GameLibSink:onChargeInfoRefeshRet(result)
	cclog("onChargeInfoRefeshRet")

end

function GameLibSink:onSendSpeakerRet(cbRetCode)
	cclog("onSendSpeakerRet")
    if (cbRetCode ~= 1) then
		--require("k510/Resources").ShowMessage("您的消息发送失败，请确保您有足够的金币,若在房间内，发言后的金币数量需高于房间下限！")
	else
		--发送大喇叭成功后，再重新获取任务列表
...
function GameLibSink:onEnteredGameRoom(tableCount,chairCount)
	cclog("onEnteredGameRoom " .. tableCount ..";"..chairCount)
...
function GameLibSink:onCreateFriendTableFailed()
	cclog("onCreateFriendTableFailed ")
...
function GameLibSink:onCreateFriendTableSuccend(exprireTime, validTime)
    ...
    require("k510/Game/GameLogic"):onFriendRuleMessage(self.game_lib:getMyself():getUserDBID(), exprireTime, validTime)
end

function GameLibSink:onDismissFriendTableMessage()
    cclog("onDismissFriendTableMessage")
...
function GameLibSink:onDismissTableNoticeMessage(userID, cbWaitTime)
    ...
    local super = require("k510/Game/GameLogic").StaySceneLayer
    if super ~= nil then
        require("Lobby/FriendGame/FriendGameLogic").showFriendVoteDlg(
                super, 0, player, cbWaitTime, self)
    end
end
...
function GameLibSink:onFriendTableVoteMessage(master, masterid, bfinish, bsuccend, voteinfo, cbWaitTime)
    ...
    if super ~= nil then
        require("Lobby/FriendGame/FriendGameLogic").addVoteResultDlg(
                        super, 0, master, masterid, voteinfo, cbWaitTime, self)
    
        if bfinish ~= 0 then
            require("Lobby/FriendGame/FriendGameLogic").showVoteResult(
                            super, 0, bsuccend, self, voteinfo)   
        end   
    end
end
...
function GameLibSink:onFriendTableRuleMessage(gamenum, options, cbMasterUserDBID, exprireTime, validTime)
    ...
    require("k510/Game/GameLogic"):onFriendRuleMessage(cbMasterUserDBID, exprireTime, validTime)    --创建成功，获取到桌子规则
    local arg = {}
 	require("GameLib/common/EventDispatcher"):instance():dispatchEvent(require("k510/Resources").NOTIFYCATION_DESKRULE, arg)
    -- table.print(FriendGameLogic.my_rule)
end

function GameLibSink:onFriendTableEndMessage(infoList)
	require("k510/Game/GameLogic"):onFriendGameEnd(infoList)
    self:exit()
end
...
function GameLibSink:onLeaveGameView()
    ...
    require("k510/Game/GameLogic"):onLeaveGameView()
    -- 因为从总结算界面返回大厅，所以现在不做任何处理
    --[[
    self:exit()
...
function GameLibSink:onRoomUserInfoUpdated(userID)
	cclog("onRoomUserInfoUpdated: "..userID)
	...
	if userInfo:getUserChair() >= 0 and pMyself:getUserTableID() == userInfo:getUserTableID() then
		require("k510/Game/GameLogic"):updateUserInfo(userInfo)
	end
...
function GameLibSink:onTableInfoChanged(tableID)
	cclog("onTableInfoChanged "..tableID)
end
...
function GameLibSink:onRecvTableChat(chat)
    ...
    local arg = {}
    arg.chat = chat
    require("GameLib/common/EventDispatcher"):instance():dispatchEvent(require("k510/Resources").NOTIFICATION_RECEIVETABLECHAT, arg)
end
...
function GameLibSink:onChargeFinished()
    ...
    require("k510/Resources").ShowMessage(("恭喜，您的充值已到帐，祝游戏愉快！"));
end
...
function GameLibSink:onGameBankOpeReturn(succeeded,gold,bank)
  	if (succeeded == 1) then
        require("k510/Resources").ShowMessage(("操作成功"))
  	else
  	    require("k510/Resources").ShowMessage(("操作失败"))
    end
end
