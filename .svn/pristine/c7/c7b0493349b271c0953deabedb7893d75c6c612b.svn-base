--TutorialLogic_SGMJ.lua

TutorialLogic_SGMJ = TutorialLogic_SGMJ or {}
...
-- 初始化教程记录(测试用)
function TutorialLogic_SGMJ:resetRecorder(t)
    -- print("TutorialLogic_SGMJ:resetRecorder")
...
-- 读取教程记录
function TutorialLogic_SGMJ:getRecorder(panel)
    ...
    local path = require("sgmj/GameDefs").CommonInfo.Img_Path
...
-- 更新教程记录
function TutorialLogic_SGMJ:setRecorder(t)
    -- print(os.time(), "记录教程 "..TutorialName[t])
    -- print("TutorialLogic_SGMJ:setRecorder", os.time(), t)
    if t == 1 then
...
-- 发牌教程判定 id:1
function TutorialLogic_SGMJ:checkSend(panel)
...
-- 摆牌教程判定 id:2
function TutorialLogic_SGMJ:checkOrder(panel)
...
-- 双击摆牌教程判定 id:3
function TutorialLogic_SGMJ:checkAutoOrder(panel)
...
-- 创建教程
TutorialLogic_SGMJ.createTutorial = {
    -- 出牌教程(已取消)
    [1] = function()
        --[[
    ...
    [2] = function()
        ...
        array2:addObject(CCCallFuncN:create(function(sender)        
            TutorialLogic_SGMJ:hideTutorial(require("sgmj/Game/GameLogic"):getInstance():getMyPanel(), 2)
        end)) 
    ...
    [3] = function()
        ...
        array2:addObject(CCDelayTime:create(TutorialAutoCloseTime))
        array2:addObject(CCCallFuncN:create(function(sender)        
            TutorialLogic_SGMJ:hideTutorial(require("sgmj/Game/GameLogic"):getInstance():getMyPanel(), 3)
        end)) 
        layer:runAction(CCSequence:create(array2))
        return layer
    end
...
-- 显示教程
function TutorialLogic_SGMJ:showTutorial(panel, t)
...
-- 关闭指定教程
function TutorialLogic_SGMJ:hideTutorial(panel, t)
...
-- 关闭所有教程
function TutorialLogic_SGMJ:hideAllTutorials(panel)
...
-- 教程检查
function TutorialLogic_SGMJ:check(panel, t)
    -- print("--------------------------------------")
...
TutorialLogic_SGMJ.checkPoint = {
    [0] = function(panel)
        TutorialLogic_SGMJ:hideAllTutorials(panel)
    end,
    [1] = function(panel)
        TutorialLogic_SGMJ:checkSend(panel)
    end,
    [2] = function(panel)
        TutorialLogic_SGMJ:hideTutorial(panel, 1)
        TutorialLogic_SGMJ:checkOrder(panel)
    end,
    [3] = function(panel)
        TutorialLogic_SGMJ:hideTutorial(panel, 2)
        TutorialLogic_SGMJ:checkAutoOrder(panel)
    end,
    [4] = function(panel)
        TutorialLogic_SGMJ:hideTutorial(panel, 3)
    end,
    [5] = function(panel)
        if panel.isStart then
            local sLogic = require("Lobby/Set/SetLogic")
            if sLogic.getOutCardType() == 1 then TutorialLogic_SGMJ:hideTutorial(panel, 1) end
            if sLogic.getCardOrderType() == 1 then TutorialLogic_SGMJ:hideTutorial(panel, 2) end
            if not TutorialLogic_SGMJ:checkSend(panel) then
                TutorialLogic_SGMJ:checkOrder(panel)
            end
        end
    end
