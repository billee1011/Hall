...
function LayerGame:init(playerNum,gameLogicInstance) 
    ...
    --标题
    local titleSp = loadSprite("sgmj/gameTitle1.png")
...
function LayerGame:initGameUI() 
	...
	CommonInfo.Logic_Path = "sgmj/Game/GameLogic"
    -- 聊天
    self:initVoiceUI()
...
function LayerGame:initFriendGameRule() --添加规则文本
   ...
   for i,v in ipairs(marksSp) do
	...
	if FriendGameLogic.getRuleValueByIndex(3) == 2 then -- 做牌
		optionName = {"GZ","QGMFB","D19","FB"}
		--table.insert(marksSp, loadSprite("sgmj/zuoPai.png"))
		local ruleData = FriendGameLogic.getRuleValueByIndex(8)--可吃胡
		if ruleData >= 1 and ruleData <= 5 then
		   table.insert(marksSp, loadSprite("sgmj/chihu_"..ruleData..".png"))
		end
