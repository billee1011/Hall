...
function ByteArray:readBuf(__len)
	...
	--print("readBuf len = " .. __len .. ",pos = " .. self._pos)
...
function ByteArray:getBuf()
	...
	self:setPos(1)
	local ret = self:readBuf(self:getLen())
	self:setPos(oldPos)
	return ret
end

function ByteArray:writeByteArray(byteArray)
	if(byteArray == nil) then
		return
	end
	self:writeBuf(byteArray:getBuf())
	return self
end

function ByteArray:initBuf(buf)
	self:setPos(1)
	self:writeBuf(buf)
	self:setPos(1)
end

function ByteArray:readStringSubZero(len)
	local str = self:readString(len)
	return self:stringSubZero(str)
end

function ByteArray:stringSubZero(src)
	local len = 0
	while len < string.len(src) do
		if string.byte(string.sub(src,len + 1)) == 0 then 
			break
		end
		len = len + 1
	end
...
function ByteArray:writeStringWithLen(str,len)
	self:writeString(str)
	for i=string.len(str) + 1,len do
		self:writeByte(0)
	end
end
