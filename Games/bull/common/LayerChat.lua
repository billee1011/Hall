    ...
    if layerChat and layerChat.view then
        ...
        if i ~= layerChat.currentPanel then
            ...
            for j = 1, 3 do
                ...
                local s = (i == opt.id) and "public/labelbg%d_selected.png" or "public/labelbg%d_normal.png"
...
function LayerChat:addEmoteDetail()
    ...
    local title = loadSprite("public/chatLab1.png")
...
function LayerChat:addChatDetail()
    ...
    local titleSz = titleBg:getContentSize()
    local title = loadSprite("public/chatRTitleBg.png")
    title:setPosition(ccp(titleSz.width / 2, titleSz.height / 2))
    titleBg:addChild(title)
        ...
        if index == 1 then
            ...
            tableView:setCreateCellFunc(function(idx)
                ...
                if idx < #quickChatCfg - 1 then
                    local sp = loadSprite("public/split.png")
            ...
            tableView:setCreateCellFunc(function(idx)
                local r = require("bull/GameLibSink").game_lib._gameRoom
    ...
    for i,v in ipairs(poses) do
        local btn = CCButton.put(titleBg, CCButton.createCCButtonByFrameName("public/chatLab"..(i + 1).."_normal.png", 
        "public/chatLab"..(i + 1).."_select.png", "public/chatLab"..(i + 1).."_select.png", function(tag, target)
        ...
        local btnSp =  loadSprite("public/chatRTitle.png")
...
function LayerChat:addTableView(ancpos, pos)
    ...
    -- 背景框
    local viewbg = loadSprite("public/chatInnerBg.png", true)
    ...
    --添加标题
    local titleBg = loadSprite("public/chatLTitle.png")
...
function LayerChat:init()
    ...
    self.bg = loadSprite("public/chatBg.png", true)
    ...
    self.input = input
    local btn_input = CCControlButton:create(loadSprite("public/btn_send.png", true))
    btn_input:setBackgroundSpriteForState(loadSprite("public/btn_send_press.png", true), CCControlStateHighlighted)
