...
function CCButton:onTouchBegan(x,y)
	...
	if self.m_normalSp:boundingBox():containsPoint(self.m_normalSp:getParent():convertToNodeSpace(self.m_touchBeginPoint))==true then
		...
		if self.m_touchDownCallBack then
			self.m_touchDownCallBack(self:getTag(),self)
		end   
		return true
	else
		return false
...
function CCButton:onEnded(x,y)
	if  self.m_bTouchMove==false then
	   	if self.m_actionCallBack and self.m_enabel==true then
	...
	if self.m_touchDownCallBack and self.m_bTouchMove==true then
		if self.m_actionCallBack then
			self.m_actionCallBack(self:getTag(),self)
		end
	end
...
function CCButton:setTouchDownCallBack(pTouchDownHandler)
	self.m_touchDownCallBack = pTouchDownHandler
end
