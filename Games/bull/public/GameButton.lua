...
function GameButton:onTouchBegan(x,y)
	...
	if self.m_pNormalBGSp:boundingBox():containsPoint(self.m_pNormalBGSp:getParent():convertToNodeSpace(self.m_touchBeginPoint))==true then
	  	...
	  	if self.m_bIsScale then
        	self.m_pPressBGSp:setScale(1.2)
        end
...
function GameButton:setEnabled(bIsEnabled)
	...
	if (bIsEnabled == true) then
		self.m_pNormalBGSp:setVisible(true)
        self.m_pNormalBGSp:setColor(self.m_Color)
		self.m_pPressBGSp:setVisible(false)
        self.m_pPressBGSp:setColor(self.m_Color)

        if (self.m_pContentNormalSp ~= nil) then
	  	    self.m_pContentNormalSp:setVisible(true)  
            self.m_pContentNormalSp:setColor(self.m_Color)
        end
        if (self.m_pContentPressSp ~= nil) then
	  	    self.m_pContentPressSp:setVisible(false)
            self.m_pContentPressSp:setColor(self.m_Color)
        end
	else
		self.m_pNormalBGSp:setVisible(false)
        self.m_pNormalBGSp:setColor(Resources.IMAGE_GRAY_COLOR)
		self.m_pPressBGSp:setVisible(true)

        if (self.m_pContentNormalSp ~= nil) then
	  	    self.m_pContentNormalSp:setVisible(false)  
            self.m_pContentNormalSp:setColor(Resources.IMAGE_GRAY_COLOR)
        end
        if (self.m_pContentPressSp ~= nil) then
	  	    self.m_pContentPressSp:setVisible(true)
        end
...
function GameButton:init(szBGUrl, pContentUrl, tag, actionCallBack)
	self.m_bIsScale = false
	if pContentUrl then
		self.m_bIsScale = false
		self.m_pNormalBGSp , self.m_pPressBGSp = Resources.getSprites(szBGUrl)
	else
		self.m_bIsScale = true
		self.m_pNormalBGSp , self.m_pPressBGSp = Resources.getSpritesEx(szBGUrl)
	end
	...
	self.m_Color = self.m_pNormalBGSp:getColor()
    ...
    if pContentUrl then
	    self.m_pContentNormalSp, self.m_pContentPressSp = Resources.getSprites(pContentUrl)
