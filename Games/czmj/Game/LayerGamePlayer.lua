...
function LayerGamePlayer.updataMusicPath()
    local check = SetLogic.getGameCheckByIndex(3)
    if check ~= 1 then
        LayerGamePlayer.music_path = CommonInfo.Music_Path
    else
    ...
    LayerGamePlayer.lan_set = AppConfig.language[check]  
end
...
function LayerGamePlayer:playVoiceEffect(index, chatid)
    local source = string.format("resources/music/quickchat/%s/%sword%d.mp3", LayerGamePlayer.lan_set, self.voice_table[index + 1], chatid)
...
function LayerGamePlayer:addUserLogo(userInfo)
    ...
    local chair = require("czmj/GamePlayLogic"):getInstance():getRelativeChair(index)
...
function LayerGamePlayer:clearAllPlayer()
    for i=1,require("czmj/Game/GameLogic"):getInstance().cbPlayerNum do
