...
local CmdInfo = require("gdmj/GameDefs").CmdInfo
...
require("gdmj/Game/TutorialLogic")
...
function GameLogic.removeCache()
    ...
    Cache.removePlist{"gdmj/images/gdmj","gdmj/images/czmjtutorial",path.."mjcard", "emote", path.."mjresult", path.."mjanima", path.."mjdesk"}
...
function GameLogic.loadingCache()
    ...
    Cache.add{"emote","gdmj/images/gdmj","gdmj/images/czmjtutorial",path.."mjcard", path.."mjresult", path.."mjanima", path.."mjdesk"}
end
...
function GameLogic:init()  
    ...
    self.cbGameCardCount = 136  --一共多少张牌
...
function GameLogic:onSceneChanged(pData, nLen)
    ...
    local function chiOperator(downcard, removeCard, dirct, index)
        ...
        local uis, pos, dirction = panel:addPengWeave(downcard, dirct)
		for i = 1,#uis do
			table.insert(self.visibleCardSpList[2],{downcard[i],uis[i]})
		end
    end
...
function GameLogic:onGameStartMessage(chair, data, nLen)
	...
	cclog("zhangshu："..self.cbGameCardCount)
   ...
   -- endInfo.cbWinUser = ba:readUShort()       --胡牌用户
    ...
    for i=1,self.cbPlayerNum do
        local score = {nTotoalScore = ba:readInt(), nHuScore = ba:readInt(), nGangScore = ba:readInt(), 
                nHorseScore = ba:readInt(), nContinuousWinScore = ba:readInt(),nPiaoScore = ba:readInt(),nCallBankerScore = ba:readInt()}
        table.insert(endInfo.UserScoreList, score)

        table.insert(endInfo.gameScores, score.nTotoalScore - score.nHorseScore)-- 
    end
...
function GameLogic:registOperatorBack()
    --吃牌操作
    local function chiOperator(downcard, removeCard, dirct, chair,provide,nSpecialMark)
        ...
        local uis,dirction = panel:onChiPengAnima(downcard, removeCard, dirct)

		for i = 1,#uis do--保存扑克精灵,用户选择牌提示的时候需要用到
			table.insert(self.visibleCardSpList[2],{downcard[i],uis[i]})
		end
		for i = #self.visibleCardSpList[1][provide+1],1,-1 do -- 删除打出去的牌
			if downcard[2] == self.visibleCardSpList[1][provide+1][i][1] then
		...
		if Bit:_and(nSpecialMark,0x0001) == 1 then  --12张落地
			self.main_layer:addJianRenMark({provide})
		elseif Bit:_and(nSpecialMark,0x0001) == 3 then --9张落地报警
			self.main_layer:showWaringTip(chair)
		end
        return removeCard
    end

    self.operatorBack_funcs[GameDefs.OperateCmd.Right_Chi] = function(chair, card, provide,nSpecialMark)
        local downcard = {card - 2, card, card - 1}
        local removeCard = {card - 2, card - 1}   

        return chiOperator(downcard, removeCard, 2, chair,provide,nSpecialMark)      
    end

    self.operatorBack_funcs[GameDefs.OperateCmd.Middle_Chi] = function(chair, card, provide,nSpecialMark)
        local downcard = {card - 1, card, card + 1}
        local removeCard = {card - 1, card + 1}
    ...
    self.operatorBack_funcs[GameDefs.OperateCmd.Left_Chi] = function(chair, card, provide,nSpecialMark)
        local downcard = {card + 1, card, card + 2}
        local removeCard = {card + 1, card + 2}
    ...
    self.operatorBack_funcs[GameDefs.OperateCmd.Chi_Hu] = function(chair, card, provide,nSpecialMark)
		...
		local huImg = {}
		nSpecialMark = Bit:_and(nSpecialMark, 0xFFFFFFE0)--屏蔽掉前五位
		
		for k,v in pairs(HuType) do
			if  Bit:_and(nSpecialMark, v) == v then
				table.insert(huImg,{string.format("gdmj/huAnim_%x.png", v),v})
			end
		end
		if #huImg > 1 then --超过一种牌型，对牌型进行排序
			table.sort(huImg, function(v, w) return v[2] > w[2] end )
		end
		tablePrint(huImg)
		local index = 0
		function playHuPaiAnm()--播放胡牌特殊牌型动画	
			index = index + 1
			if index <= #huImg then
				self.main_layer:playHuTypeAnima(chair,huImg[index][1],playHuPaiAnm)
			end	
		end
		self.main_layer:playOperatorAnima(imgHu, chair, playHuPaiAnm)
    end
end
function GameLogic:addDirectIcon(szType,cardSp,chair,provide)
	...
	--if szType == "Peng" or szType == "PengGang" or szType == "Gang" then
		...
		if nPos == 2 or nPos == 4 then --左右的位置
			cardSp.directArrowSp = loadSprite(dirct == 2 and "gdmj/HDirect_Right.png" or "gdmj/VDirect_Right.png")
			...
			cardSp.directArrowSp = loadSprite("gdmj/DirectArrow.png")
			cardSp.card_bg:addChild(cardSp.directArrowSp)
			cardSp.directArrowSp:setPosition(poes[nPos])
...
--[[	local card = { 0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,
	...
	self.cardLayer1 = require("gdmj/Game/LayerMyCard").put(self.main_layer, self.main_layer.downcard_zIndex + 4)
	self.cardLayer2 = require("gdmj/Game/LayerSouthCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	self.cardLayer3 = require("gdmj/Game/LayerNorthCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	self.cardLayer4 = require("gdmj/Game/LayerWestCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	local uis = self.cardLayer1:addCardWall(17)
	uis[1]:getParent():setVisible(false)
	uis = self.cardLayer2:addCardWall(17)
	uis[1]:getParent():setVisible(false)
	uis = self.cardLayer3:addCardWall(17)
	uis[1]:getParent():setVisible(false)
	uis = self.cardLayer4:addCardWall(17)
	uis[1]:getParent():setVisible(false)
	...
	for i = 1,27 do
		...
		local item ,pos = self.cardLayer3:addStaticPassed(card[i])
		self.cardLayer4:addStaticPassed(card[i])
...
--[[	self.cardLayer2 = require("gdmj/Game/LayerSouthCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	self.cardLayer3 = require("gdmj/Game/LayerNorthCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	self.cardLayer4 = require("gdmj/Game/LayerWestCard").put(self.main_layer, self.main_layer.downcard_zIndex)
	self.cardLayer3:addCardWall(17)
	self.cardLayer1:addCardWall(17)
	self.cardLayer2:addCardWall(15)
	self.cardLayer4:addCardWall(13)]]
	
--[[	local sp = require("gdmj/Game/VerticalShare").create()
	sp:setRotation(-90)
...
--	self.main_layer:playHuTypeAnima(0,"gdmj/huAnim_100.png",function() end)
--[[	local SpriteWestCard = require("gdmj/Game/SpriteWestCard")
