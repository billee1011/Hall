    ...
    if layerChat and layerChat.view then
        ...
        if i ~= layerChat.currentPanel then
            ...
            for j = 1, 3 do
                ...
                local s = (i == opt.id) and "gdmj/labelbg%d_selected.png" or "gdmj/labelbg%d_normal.png"
...
function LayerChat:addEmoteDetail()
    ...
    local title = loadSprite("gdmj/chatLab1.png")
...
function LayerChat:addChatDetail()
    ...
    local titleSz = titleBg:getContentSize()
    local title = loadSprite("gdmj/chatRTitleBg.png")
    title:setPosition(ccp(titleSz.width / 2, titleSz.height / 2))
    titleBg:addChild(title)
        ...
        if index == 1 then
            ...
            tableView:setCreateCellFunc(function(idx)
                ...
                if idx < #quickChatCfg - 1 then
                    local sp = loadSprite("gdmj/split.png")
            ...
            tableView:setCreateCellFunc(function(idx)
                ...
                if chat then
                    ...
                    for k, v in pairs(chat) do
                        local chair = require("gdmj/Game/GameLogic"):getInstance():getRelativeChair(k)
    ...
    for i,v in ipairs(poses) do
        local btn = CCButton.put(titleBg, CCButton.createCCButtonByFrameName("gdmj/chatLab"..(i + 1).."_normal.png", 
        "gdmj/chatLab"..(i + 1).."_select.png", "gdmj/chatLab"..(i + 1).."_select.png",function(tag, target)
        ...
        local btnSp =  loadSprite("gdmj/chatRTitle.png")
...
function LayerChat:addTableView(ancpos, pos)
    ...
    -- 背景框
    local viewbg = loadSprite("gdmj/chatInnerBg.png", true)
    ...
    --添加标题
    local titleBg = loadSprite("gdmj/chatLTitle.png")
...
function LayerChat:init()
    ...
    self.bg = loadSprite("gdmj/chatBg.png", true)
    ...
    self.input = input
    local btn_input = CCControlButton:create(loadSprite("gdmj/btn_send.png", true))
    btn_input:setBackgroundSpriteForState(loadSprite("gdmj/btn_send_press.png", true), CCControlStateHighlighted)
