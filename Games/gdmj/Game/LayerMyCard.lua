...
function LayerMyCard:init() 
	...
	self.bCanHu = false -- 能不能胡牌
    self:test()
end
--添加操作按钮{{card - 2, card - 1, card}, {card - 1, card, card + 1}, {card, card + 1, card + 2}}
function LayerMyCard:addChiBtn(types, card)
    --判断是否没有选择
    local indexs = {}
    ...
    local itemIndex = 1
    if #indexs > 1 then
        local chiCards = {{card, card + 1, card + 2}, {card - 1, card, card + 1}, {card - 2, card - 1, card}}
        self:addFlickerBtn("chi", function()
            ...
            for i,v in ipairs(indexs) do
                local item = createButtonWithFilePath("mjdesk/chiBg.png", nil, function()
                        require(CommonInfo.Logic_Path):getInstance():sendOperateCard(math.pow(2,v - 1))
                        self:clearOperatorBtn()
                    end, nil, CCSizeMake(198, 106))
                ...
                --创建麻将子
                for j,w in ipairs(chiCards[v]) do
                    local mjSp = SpriteGameCard.createHand(w)
                    mjSp:setScale(0.7)
        ...
        self:addFlickerBtn("chi",function()
            require(CommonInfo.Logic_Path):getInstance():sendOperateCard(math.pow(2,indexs[1] - 1))
            self:clearOperatorBtn()
        end)           
    end
end
...
function LayerMyCard:addFlickerBtn(img, func)
    if TutorialLogic_GDMJ then
        TutorialLogic_GDMJ:check(self, 0)
    end
	if img == "hu" or img == "zimo" then self.bCanHu = true end --能够胡牌
    local item, s9Sp = createButtonWithOneStatusFrameName("mjAnima/btn_operator_"..img.."1.png", nil, function()
			if self.bCanHu and (img ~= "hu" and img ~= "zimo")  then
				--能胡牌而不胡的情况弹框提示
				local parent = self:getParent()
				parent:popMessageBox("您当前可以胡牌，是否放弃？",function(bOk)
					if bOk and func ~= nil then
						func()
					end
				end,parent,self.btn_zIndex + 1)
			else
				func()
			end
        end)
...
function LayerMyCard:gameStart(func) 
    ...
    array:addObject(CCCallFunc:create(function()        
        ...
        self:initTouch()
        if TutorialLogic_GDMJ then
            TutorialLogic_GDMJ:check(self, 5)
        end
        func()
    end)) 
    ...
    if SetLogic.getOutCardType() == 1 then
        if self.operate_cards:boundingBox():containsPoint(point) then
            ...
            if seces <= self.touch_seces + 1 then
                ...
                self.touch_seces = 0
                if TutorialLogic_GDMJ then
                    TutorialLogic_GDMJ:check(self, 4)
                end  
            else
    ...
    -- 拖曳出牌
        ...
        if Logic:mineIsSender() and self.card_index then
            ...
            if card and (card:getPositionY() >= self.outline or seces <= card.touch_seces + 1) then
				...
				if self.bCanHu then --能胡牌而不胡的情况弹框提示		
					local parent = self:getParent()
					parent:popMessageBox("您当前可以胡牌，是否放弃？",function(bOk)
						if bOk then
        ...
        if self.card_index then
            ...
            if self.card_index ~= tIdx then 
                if TutorialLogic_GDMJ then
                    TutorialLogic_GDMJ:check(self, 3)
                end
            end
...
function LayerMyCard:createPassed(card)
	...
	sp, downpos, scale, zIndex = self:getPassedPos(passIndex, card)
...
function LayerMyCard:getPassedPos(passIndex, card)
	...
	if self.passCardPosX == nil then --每张对应的位置，每行的Y坐标一样
		self.passCardPosX = {{569.65,618.63,664.55,714.55,763.53,813.53,862.51,911.48,960.46,1009.44,1058.42,1107.40,1156.38,1205.36},--1 line
							{571,619,664,713,761,810,858,906,954}, -- 2 line
							{572.32,619.35,663.45,711.47,758.51,806.53,853.57,900.61,947.65}, --3 line
							{573.61,619.71,662.93,709.99,756.08,803.14,849.24,895.34,941.44}, --4 line
							}
		self.passCardPosY ={114.81,172.97,229.97,284.71}
	end
    local downIndex = self:getParent().downIndex
    local spaceIndex = 0
	passIndex = passIndex > 40 and 40 or passIndex -- 最多放41张牌
	
    local xIndex, yIndex = passIndex % downIndex + 1, 4 - math.floor(passIndex / downIndex)--几行几列
    if yIndex <= 0 then xIndex, yIndex = 9 + xIndex, 1 end

    local sp, index, zindex = SpriteGameCard.createVerticalPassed(card, 15 - xIndex)
    local scales = {{1/0.98, 1/0.98},{1, 1}, {0.98, math.pow(0.98, 2)}, {math.pow(0.98, 2), math.pow(0.98, 3) * 0.90}}

	self.down_pos.x = self.passCardPosX[yIndex][xIndex]
	self.down_pos.y = self.passCardPosY[yIndex]
    return sp, self.down_pos, scales[yIndex], xIndex + spaceIndex
end
--[[function LayerMyCard:getPassedPosNew(passIndex,card)--lsk add
	local downIndex = self:getParent().downIndex
	passIndex = passIndex > 40 and 40 or passIndex -- 最多放41张牌
	
    local xIndex, yIndex = passIndex % downIndex + 1, math.floor(passIndex / downIndex) + 1--几行几列
    if yIndex > 4 then xIndex, yIndex = 9 + xIndex, 4 end
	local sp, index, zindex = SpriteGameCard.createVerticalPassed(card, 12)
    local scales = {{math.pow(0.98, 2), math.pow(0.98, 3) * 0.90},{0.98, math.pow(0.98, 2)},{1, 1},{1/0.98, 1/0.98}} 
--	sp:setScaleX(scales[yIndex][1])
	sp:setScaleY(scales[yIndex][2])
	--sp:setSkewY(skewValue[xIndex]/2)
	local skewValue = {1,1,1,1,0,-1,-1,-1,-1}
	sp:setSkewX(skewValue[xIndex > #skewValue and #skewValue or xIndex]*2)
	
	sp:setAnchorPoint(ccp(0,1))
	
	local posesX = {573.61,570.61,567.61,564.61}
	local posesY = {284,229.97,172.97,114.81}
	--sp:setPosition(ccp(posesX[yIndex] + scales[yIndex][1]*(xIndex - 1)*45,posesY[yIndex]))
	
	self.down_pos.x = posesX[yIndex] + scales[yIndex][1]*(xIndex - 1)*45
	self.down_pos.y = posesY[yIndex]
	--self.down_cards:addChild(sp,passIndex)
	
	--self.passed_count = self.passed_count + 1
	
	--self.down_card = sp
	return sp, self.down_pos, scales[yIndex], 200 - self.passed_count
end]]
--场景恢复添加打出去的牌
...
function LayerMyCard:getHandPaisAnima(func, bBanker)
    ...
    SpriteGameCard.getHandPaisAnima(self, 50, function()
        ...
        if bBanker then
            if TutorialLogic_GDMJ then
                TutorialLogic_GDMJ:check(self, 1)
            end            
...
function LayerMyCard:sendCardAnima(card)
    ...
    -- 关闭发牌教程、检查摆牌教程
    if TutorialLogic_GDMJ then
        TutorialLogic_GDMJ:check(self, 2)
    end
