...
function LayerWestCard:addCardWall(count)
	...
	--微调位置
	pos.x = pos.x + (count - 14)*2
	pos.y = pos.y + (count - 14)*8
	
    for i= 14 - count + 1,14 do
        local sp, spaces, zindex, index
        for j=1,2 do
            sp, spaces, zindex, index = self.sprite_card.createWallCard(i < 1 and 1 or i, #uis)
...
--[[function LayerWestCard:createPassedNew(card)
	...
	local sp, spaces, zindex, skewIndex = self.sprite_card.createThreePassed(card, 1) 
	local posesX = {535,533,531,529,527,525,523,521,519}
	local posesY = {462}
	
	local size = sp.card_bg:getContentSize()
	local width,height= size.width*(0.83 + 0.01*xIndxe),size.height*0.6
	
	self.down_pos.x = posesX[xIndxe] - width*(yIndex - 1)
    self.down_pos.y = posesY[1] - (xIndxe - 1)*height
	
    sp:setPosition(self.down_pos)	
    self.down_cards:addChild(sp,(5 - yIndex)*100 + xIndxe) 

    self.passed_count = self.passed_count + 1

    return sp, self.down_pos,  (5 - yIndex)*100 + xIndxe
end]]
...
function LayerWestCard:addStaticPassed(card)
    --放下牌
    local item,pos = self:createPassed(card)
    self.down_card = item
	return item,pos
end
