...
function LayerDeskRecord:createRecordCellItem(info, idx)
    ...
    --游戏人数
    local playerNum = 4
    if info.GameID == 23 then playerNum = 3 end
...
function LayerDeskRecord:createDetailCellItem(info, idx)
    ...
    --游戏人数
    local playerNum = 4
    if FriendGameLogic.game_id == 23 then playerNum = 3 end
    ...
    local playBtn = CCButton.put(item, CCButton.createCCButtonByFrameName("lobbyDlg/btn_recorder_play1.png", 
            ...
            FriendGameLogic.getGamePlayRecord(0, info["ServerBatchKey"], function(gamedata)
					tablePrint(gamedata)
                    require("gdmj/GamePlayLogic"):getInstance():replaceMainScence(gamedata)
                end)
...
function LayerDeskRecord:initDetailTitle(super, data)
    ...
    local playerNum = 3
    if FriendGameLogic.game_id == 23 then playerNum = 2 end
	cclog("initDetailTitle"..playerNum)
	cclog(FriendGameLogic.game_id)
	tablePrint(data)
    for i=1,playerNum do
