...
function ShareFuns.CardValue(CardIndex)
	if not(CardIndex >= 0 and CardIndex <= 53) then
		...
		elseif pos2 == 1 then
			if ShareFuns.CardValue(cardArray[#cardArray]) == 1 then -- 有2必定有3,否则不为顺子
				...
				if posA < 0 then
					...
					local tmpArray = {}
					for i = 2,#cardArray do
						table.insert(tmpArray,cardArray[i])
					end
				...
				elseif posA == 2 then
					...
					for j = 1,2 do
						table.insert(tmpArray,cardArray[j])
					end
					cardArray = {}
					cardArray = ShareFuns.Copy(tmpArray)					
					return true,cardArray
	...
	if bRet then
		if #ArrayIn%2 == #ArrayOut%3 then
			...
			if ret1 and ret2 then
				...
				cardArray = ShareFuns.Append(cardArray,douArray)
				return true,cardArray
			end	
		end
	end
...
function ShareFuns.IsFiveTenKing(cardArray)
	if #cardArray == 3 then
		if ShareFuns.CardValue(cardArray[1]) + ShareFuns.CardValue(cardArray[2]) + ShareFuns.CardValue(cardArray[3]) == 22 then
			if ShareFuns.CardValue(cardArray[1]) == 11 and ShareFuns.CardValue(cardArray[2]) == 8 then
...
function ShareFuns.GetCardSuitType(cardArray,bombStatus)
    ...
    elseif ShareFuns.IsFiveTenKing(tempArray) then
		retSt = Common.SUIT_TYPE.suitFiveTenKing
	elseif ShareFuns.IsPureFiveTenKing(tempArray) then
		retSt = Common.SUIT_TYPE.suitPureFiveTenKing
    elseif ShareFuns.IsRedJoker(tempArray) then
		retSt = Common.SUIT_TYPE.suitRedJoker
	else
		local bRet = false
		local outArray = {}
...
-- 分解出牌型(checked)
function ShareFuns.DisassembleToOutCard(cardArray,bombStatus)
	local OutCard = {}
	local st,cardArray = ShareFuns.GetCardSuitType(cardArray,bombStatus)
...
function ShareFuns.CompareTwoCardSuit(prevOutcard,followOutCard,straightMax)
	if prevOutcard.suitType == followOutCard.suitType then--牌型相同
		if prevOutcard.suitType == Common.SUIT_TYPE.suitDoubleStraight or prevOutcard.suitType == Common.SUIT_TYPE.suitTriStraight or prevOutcard.suitType == Common.SUIT_TYPE.suitPlane then
		...
		elseif prevOutcard.suitType == Common.SUIT_TYPE.suitStraight then
		cclog("-----------%s-------------%s----",ShareFuns.CardValue(prevOutcard.cards[1]),ShareFuns.CardValue(followOutCard.cards[1]))
			if ShareFuns.CardValue(prevOutcard.cards[1]) < ShareFuns.CardValue(followOutCard.cards[1]) then --比较第一张
				return true
			else
			cclog("---------------43--------")
...
function ShareFuns.GetTriple( cardArrayIn,nCompare)
	...
	if bRet then
		if nCompare == -1 then
			table.insert(cardArrayOut,ArrayOut[#ArrayOut])
			table.insert(cardArrayOut,ArrayOut[#ArrayOut - 1])
			table.insert(cardArrayOut,ArrayOut[#ArrayOut - 2])
			return true,cardArrayOut
		end
		for i = #ArrayOut, 3, -1 do
			-- 比较大小
	...
	if bRet and #ArrayOut >= 4 then
		...
		for j = #ArrayOut,4,-1 do
			...
			if bombStatus == Common.BOMB_STATUS.BombStatus_QuadrupleOne then
				local ret,singleArray = ShareFuns.GetSingle(ArrayIn,-1,-1)
				if ret then
			...
			local ret = false
			ret,bombArray = ShareFuns.IsBomb(bombArray,bombStatus)
			if ret then
				if nCompare == -1 then
...
function ShareFuns.GetFiveTenKing(cardArrayIn)
	local cardArrayOut = {}
	...
	if isHaveFive and isHaveKing and isHaveTen then
		for i = 1,#arrayKing do
			for j = 1,#arrayTen do
				if ShareFuns.CardType(arrayKing[i]) ~= ShareFuns.CardType(arrayTen[j]) then
					...
					table.insert(cardArrayOut,arrayFive[1])
					return true,cardArrayOut
				end
			end
		end
		...
		for i = 1,#arrayFive do
			if ShareFuns.CardType(arrayKing[1]) ~= ShareFuns.CardType(arrayTen[1]) then
				table.insert(cardArrayOut,arrayKing[1])
				table.insert(cardArrayOut,arrayTen[1])
...
function ShareFuns.GetPureFiveTenKing(cardArrayIn)
	local cardArrayOut = {}
	if #cardArrayIn < 3 then
		return false,cardArrayOut
	end
...
function ShareFuns.GetRedJoker(cardArrayIn)
	local cardArrayOut = {}
...
function ShareFuns.TipOutCard(lastOutCard,cardArrayHold,bombStatus,straightMax)
    ...
    if lastSt == Common.SUIT_TYPE.suitInvalid or lastSt == Common.SUIT_TYPE.suitPass then
    	return false,cardArrayOut
    end
    ...
    if lastSt ~= Common.SUIT_TYPE.suitBomb then
		bRet,cardArrayOut = ShareFuns.GetFiveTenKing(arrayInHold)
    	if bRet then
    		return true,cardArrayOut
		else
			bRet,cardArrayOut = ShareFuns.GetPureFiveTenKing(arrayInHold)
