...
function LayerVoice:init()
    ...
    if GameLibSink.game_lib and GameLibSink.game_lib:getMyself() then
        ...
        -- 语音按钮
        local vButton = CCControlButton:create(loadSprite("common/btn_voice.png", true))
        vButton:setPreferredSize(CCSizeMake(85,86))
        vButton:setBackgroundSpriteForState(loadSprite("common/btn_voice2.png", true), CCControlStateHighlighted)
        vButton:setPosition(ccp(CommonInfo.View_Width - 50, 200))
        ...
        bg:setPosition(ccp(CommonInfo.View_Width/2, 255))
        vPanel1:addChild(bg)
        local mic = loadSprite("common/mic.png")
        mic:setPosition(ccp(CommonInfo.View_Width/2-30, 260))
        vPanel1:addChild(mic)
        for i = 1, 5 do
            local v = loadSprite(string.format("common/volume%d.png",i))
            v:setAnchorPoint(ccp(0,0))
            v:setPosition((ccp(CommonInfo.View_Width/2+10, 190 + i*20)))
            vPanel1:addChild(v)
            vPanel1["volume"..i] = v
        end
        local tip = loadSprite("common/tip_cancel.png")
        tip:setPosition(ccp(CommonInfo.View_Width/2, 190))
        ...
        arrow:setPosition(ccp(CommonInfo.View_Width/2-10, 265))
        ...
        arr:addObject(CCCallFunc:create(function()
            ...
            if c and #c.recordList > 0 and (not c.isPlaying) then
                ...
                for k, v in pairs(record) do
                    local index = require(require("sgmj/GameDefs").CommonInfo.GameLib_File).game_lib:getUser(k):getUserChair()
                    local chair = require("sgmj/Game/GameLogic"):getInstance():getRelativeChair(index)
                    c.voiceIcon:setFlipX(false)
                    if chair ~= 2 then c.voiceIcon:setFlipX(true) end
