...
function SpriteGameCard:initMyHand(index)
   ...
   -- if index == 0x35 then
    if require(CommonInfo.Logic_Path):getInstance():isSpecialCard(index) then
        --增加赖子标示
        self.laizi_mark = loadSprite("mjValue/markGui.png")
        self.card_bg:addChild(self.laizi_mark,2,30)
...
function SpriteGameCard:updataValue(index)
	...
	if require(CommonInfo.Logic_Path):getInstance():isSpecialCard(self.card_index) then
		if self.laizi_mark then
			self.laizi_mark:removeFromParentAndCleanup(true)
	    end
    --elseif index == 0x35 then
	elseif require(CommonInfo.Logic_Path):getInstance():isSpecialCard(index) then
        self.laizi_mark = loadSprite("mjValue/markGui.png")
        self.card_bg:addChild(self.laizi_mark,2,30)
        self.laizi_mark:setPosition(ccp(self.base_size.width - 18, self.base_size.height - 35))
    end
...
function SpriteGameCard:onBtnCard(state)
	...
	local stateChange = state ~= self.card_state
	...
	if stateChange then -- 牌的状态发生变化
		require(CommonInfo.Logic_Path):getInstance():onCardStateChange(self.card_state,self.card_index)
	end
end
...
-----------------------------------------------------------------------------------------------------------------------------
function SpriteGameCard:initHorizontalPassed(index, ctype,dirct)-- 这里是碰牌横放的牌
	...
	--[[if dirct and dirct >= 1 and dirct <= 3 then 
		...
		--self.directArrowSp:setScale(0.8)	
		self.directArrowSp:setPosition(ccp(self.base_size.width/2 , self.base_size.height + 14))
	end]]
...
function SpriteGameCard:initVerticalPassed(index, ctype) --自己竖的牌
    ...
    self:addChild(self.sprite_card,0,20)
...
function SpriteGameCard:changeCardToFront(index, pos)
    ...
    self.card_front:addChild(sprite_card,1,20)
