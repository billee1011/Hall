...
function LayerDeskResult:init(info)   
	...
	self.verticalShare  = require("sgmj/Game/VerticalShare").create() --竖屏分享
    --背景185
    ...
    for i,v in ipairs(info) do
		...
		self.verticalShare:createInfoItem(info[i], i)
    end
        ...
        if maxval > 0 then
            for j,w in ipairs(v) do
                ...
                if w[2] == maxval and not statues[index] then
					...
					self.verticalShare:addMarks(index,i) -- 添加最大赢家和输家
                end
            end
        end
    end
    ...
    local shareBtn = CCButton.put(self.panel_bg, CCButton.createCCButtonByFrameName("mjResult/btnShare1.png", 
            "mjResult/btnShare2.png", "mjResult/btnShare1.png", function()
                   --[[ for i,v in ipairs(self.btn_uis) do
                    ...
                    shareScreenToWx(1, 75, CommonInfo.Game_Name, "好友卓总战绩", function()end)
					...
					self.verticalShare:setVisible(true)
					shareScreenToWxEx(self.verticalShare, 1, CommonInfo.Game_Name, "好友卓总战绩", function() 
...
function LayerDeskResult:addGameMark()   
	...
	ruletext1 = ruletext1..","..ruletext2
    local msg = roomtext.."\n" --..CommonInfo.Game_Name.."："
    msg = msg..ruletext1--.."，"..birdtext
